...
function abrirModalAdicionar() {
    ...
    // Resetar categoria para valor padrão
    const categoriaSelect = document.getElementById('novoCategoriaProduto');
...
function validarFormularioAdicionar() {
    ...
    if (!nome) {
        mostrarErro('erroNomeProduto', 'Nome do produto é obrigatório');
        valido = false;
    } else if (nome.length < 3) {
        mostrarErro('erroNomeProduto', 'Nome deve ter pelo menos 3 caracteres');
        valido = false;
    }
    ...
    if (isNaN(preco) || preco <= 0) {
        mostrarErro('erroPrecoProduto', 'Preço deve ser maior que zero');
        valido = false;
    }
    ...
    if (!descricao) {
        mostrarErro('erroDescricaoProduto', 'Descrição é obrigatória');
        valido = false;
    } else if (descricao.length < 10) {
        mostrarErro('erroDescricaoProduto', 'Descrição deve ter pelo menos 10 caracteres');
        valido = false;
    }
    ...
    if (!inputImagem.files[0]) {
        mostrarErro('erroImagem', 'Imagem do produto é obrigatória');
        valido = false;
    } else {
...
function validarFormularioEditar() {
    ...
    if (!nome) {
        mostrarErro('erroNomeProdutoEditar', 'Nome do produto é obrigatório');
        valido = false;
    } else if (nome.length < 3) {
        mostrarErro('erroNomeProdutoEditar', 'Nome deve ter pelo menos 3 caracteres');
        valido = false;
    }
    ...
    if (isNaN(preco) || preco <= 0) {
        mostrarErro('erroPrecoProdutoEditar', 'Preço deve ser maior que zero');
        valido = false;
    }
    ...
    if (!descricao) {
        mostrarErro('erroDescricaoProdutoEditar', 'Descrição é obrigatória');
        valido = false;
    } else if (descricao.length < 10) {
        mostrarErro('erroDescricaoProdutoEditar', 'Descrição deve ter pelo menos 10 caracteres');
        valido = false;
    }
    
    // Validar imagem (apenas se foi selecionada uma nova)
...
function mostrarErro(elementoId, mensagem) {
    ...
    if (elemento) {
        ...
        // Adicionar classe de erro ao input pai
...
function limparErros(modalId) {
    const modal = document.getElementById(modalId);
    if (modal) {
        // Limpar todas as mensagens de erro
        const erros = modal.querySelectorAll('.error-message');
        erros.forEach(erro => {
            erro.textContent = '';
...
function limparErroIndividual(elementoId) {
    const elemento = document.getElementById(elementoId);
    if (elemento) {
        elemento.textContent = '';
        elemento.style.display = 'none';
        
        // Remover classe de erro do input relacionado
        const input = elemento.previousElementSibling;
        if (input && (input.tagName === 'INPUT' || input.tagName === 'SELECT' || input.tagName === 'TEXTAREA')) {
            input.classList.remove('input-error');
        }
...
function adicionarEventListenersValidacao() {
    // Validação em tempo real para o modal de adicionar
    const inputsAdicionar = [
        { id: 'novoNomeProduto', erroId: 'erroNomeProduto' },
        { id: 'novoPrecoProduto', erroId: 'erroPrecoProduto' },
        { id: 'novoCategoriaProduto', erroId: 'erroCategoriaProduto' },
        { id: 'novaDescricaoProduto', erroId: 'erroDescricaoProduto' },
        { id: 'novaImagemProduto', erroId: 'erroImagem' }
    ];
    
    inputsAdicionar.forEach(inputConfig => {
        const input = document.getElementById(inputConfig.id);
        if (input) {
            input.addEventListener('blur', function() {
                // Validar individualmente quando o usuário sai do campo
                if (this.value.trim() !== '' || this.files[0]) {
                    limparErroIndividual(inputConfig.erroId);
                }
            });
        }
    });
    
    // Validação em tempo real para o modal de editar
    const inputsEditar = [
        { id: 'nomeProduto', erroId: 'erroNomeProdutoEditar' },
        { id: 'precoProduto', erroId: 'erroPrecoProdutoEditar' },
        { id: 'categoriaProduto', erroId: 'erroCategoriaProdutoEditar' },
        { id: 'descricaoProduto', erroId: 'erroDescricaoProdutoEditar' },
        { id: 'imagemProduto', erroId: 'erroImagemEditar' }
    ];
    
    inputsEditar.forEach(inputConfig => {
        const input = document.getElementById(inputConfig.id);
        if (input) {
            input.addEventListener('blur', function() {
                // Validar individualmente quando o usuário sai do campo
                if (this.value.trim() !== '' || (this.type === 'file' && this.files[0])) {
                    limparErroIndividual(inputConfig.erroId);
                }
            });
        }
