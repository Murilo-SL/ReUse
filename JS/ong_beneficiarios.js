        ...
        <div class="beneficiary-card" data-type="${beneficiary.type}">
            ...
            <div class="beneficiary-footer">
                ...
                <div class="beneficiary-actions">
                    ...
                    <button class="action-btn delete" title="Excluir" onclick="confirmDelete('${beneficiary.id}', '${beneficiary.name.replace(/'/g, "\\'")}')">
...
function openAddBeneficiaryModal() {
    ...
    clearAllFormErrors('beneficiary-form');
...
function validarFormulario(formId) {
    ...
    const isEditForm = formId === 'beneficiary-edit-form';
    const prefix = isEditForm ? 'edit-beneficiary-' : 'beneficiary-';
    
    const fields = [
        { id: `${prefix}name`, label: 'Nome', required: true },
        { id: `${prefix}type`, label: 'Tipo', required: true },
        { id: `${prefix}email`, label: 'E-mail', required: true, validator: isValidEmail },
        { id: `${prefix}phone`, label: 'Telefone', required: true, validator: isValidPhone },
        { id: `${prefix}address`, label: 'Endereço', required: true },
        { id: `${prefix}city`, label: 'Cidade', required: true },
        { id: `${prefix}state`, label: 'Estado', required: true },
        { id: `${prefix}animals`, label: 'Número de animais', required: true, validator: isValidAnimals },
        { id: `${prefix}status`, label: 'Status', required: true }
    ];
    ...
    fields.forEach(field => {
        ...
        if (field.validator && value) {
            const validationResult = field.validator(value, field.label);
            if (validationResult !== true) {
                showFieldError(field.id, validationResult);
                isValid = false;
            }
        }
    });
...
// Função para limpar todos os erros de um formulário
...
// Validar e-mail
function isValidEmail(email, fieldLabel) {
...
// Validar telefone
function isValidPhone(phone, fieldLabel) {
...
// Validar número de animais
function isValidAnimals(animals, fieldLabel) {
    const num = parseInt(animals);
    if (isNaN(num)) {
        return 'Número de animais deve ser um valor numérico';
    }
    
    if (num < 1) {
        return 'Número de animais deve ser no mínimo 1';
    }
    
    if (num > 999) {
        return 'Número de animais deve ser no máximo 999';
    }
...
function validateSingleField(fieldId) {
    const fieldConfig = {
        'beneficiary-name': { label: 'Nome', required: true },
        'beneficiary-type': { label: 'Tipo', required: true },
        'beneficiary-email': { label: 'E-mail', required: true, validator: isValidEmail },
        'beneficiary-phone': { label: 'Telefone', required: true, validator: isValidPhone },
        'beneficiary-address': { label: 'Endereço', required: true },
        'beneficiary-city': { label: 'Cidade', required: true },
        'beneficiary-state': { label: 'Estado', required: true },
        'beneficiary-animals': { label: 'Número de animais', required: true, validator: isValidAnimals },
        'beneficiary-status': { label: 'Status', required: true },
        'edit-beneficiary-name': { label: 'Nome', required: true },
        'edit-beneficiary-type': { label: 'Tipo', required: true },
        'edit-beneficiary-email': { label: 'E-mail', required: true, validator: isValidEmail },
        'edit-beneficiary-phone': { label: 'Telefone', required: true, validator: isValidPhone },
        'edit-beneficiary-address': { label: 'Endereço', required: true },
        'edit-beneficiary-city': { label: 'Cidade', required: true },
        'edit-beneficiary-state': { label: 'Estado', required: true },
        'edit-beneficiary-animals': { label: 'Número de animais', required: true, validator: isValidAnimals },
        'edit-beneficiary-status': { label: 'Status', required: true }
    };
    ...
    if (config.required && !value) {
        showFieldError(fieldId, `${config.label} é obrigatório`);
        return false;
    }
    
    // Validadores específicos
    if (config.validator && value) {
        const validationResult = config.validator(value, config.label);
        if (validationResult !== true) {
            showFieldError(fieldId, validationResult);
            return false;
        }
    }
...
function openEditBeneficiaryModal(id) {
    ...
    if (beneficiary) {
        ...
        // Limpar todos os erros
        clearAllFormErrors('beneficiary-edit-form');
...
function exportBeneficiaries() {
    try {
        ...
        console.error('Erro ao exportar beneficiários:', error);
        mostrarNotificacao('Erro ao exportar beneficiários', 'error');
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const beneficiaryEditForm = document.getElementById('beneficiary-edit-form');
    const confirmDeleteBtn = document.getElementById('confirm-delete');
    
    if (beneficiaryForm) {
        beneficiaryForm.addEventListener('submit', saveBeneficiary);
    }
    
    if (beneficiaryEditForm) {
        beneficiaryEditForm.addEventListener('submit', saveEditBeneficiary);
    }
    
    if (confirmDeleteBtn) {
        confirmDeleteBtn.addEventListener('click', deleteBeneficiary);
    }
    ...
    const refreshBtn = document.getElementById('btn-refresh-beneficiarios');
    if (refreshBtn) {
    ...
    const validationFields = [
        'beneficiary-name', 'beneficiary-type', 'beneficiary-email', 'beneficiary-phone',
        'beneficiary-address', 'beneficiary-city', 'beneficiary-state', 'beneficiary-animals', 'beneficiary-status',
        'edit-beneficiary-name', 'edit-beneficiary-type', 'edit-beneficiary-email', 'edit-beneficiary-phone',
        'edit-beneficiary-address', 'edit-beneficiary-city', 'edit-beneficiary-state', 'edit-beneficiary-animals', 'edit-beneficiary-status'
    ];
    ...
    validationFields.forEach(fieldId => {
        ...
        if (field) {
            field.addEventListener('blur', function() {
                // Validar campo individualmente quando perde o foco
    ...
    const applyAdvancedFiltersBtn = document.querySelector('.advanced-filters .primary-button');
    if (applyAdvancedFiltersBtn) {
        applyAdvancedFiltersBtn.addEventListener('click', aplicarFiltros);
    }
