...
// Funções de cálculo de dados para gráficos - NOVAS
...
function atualizarResumoEstatistico() {
    ...
    // Adicionar novos elementos se não existirem
    if (!document.getElementById('valor-total')) {
        criarNovosElementosResumo(valorTotal, categoriasUnicas);
    } else {
...
function criarNovosElementosResumo(valorTotal, categoriasUnicas) {
    const summaryCards = document.querySelector('.summary-cards');
    if (summaryCards) {
        summaryCards.innerHTML += `
            <div class="summary-card">
                <div class="summary-value" id="valor-total">R$ ${valorTotal.toLocaleString('pt-BR')}</div>
                <div class="summary-label">Valor Total Estimado</div>
            </div>
            <div class="summary-card">
                <div class="summary-value" id="categorias-unicas">${categoriasUnicas}</div>
                <div class="summary-label">Categorias Diferentes</div>
            </div>
        `;
    }
}
...
// Função principal para gerar relatório - NOVA
...
function aplicarFiltros() {
    ...
    switch (tipoRelatorio) {
        case 'distribuicao':
            ...
            break;
        case 'categorias':
            // Agrupar por categoria para relatório de categorias
            dadosFiltrados = agruparPorCategoria(dadosFiltrados);
            break;
        case 'doadores':
            // Agrupar por doador para relatório de doadores
            dadosFiltrados = agruparPorDoador(dadosFiltrados);
            break;
        case 'doacoes':
        default:
            // Mantém todos os dados para relatório de doações
            break;
    }
...
function agruparPorCategoria(dados) {
    ...
    dados.forEach(item => {
        ...
        agrupado[item.categoria].quantidade += item.quantidade;
        agrupado[item.categoria].valorTotal += (item.valorEstimado || 0);
        agrupado[item.categoria].doacoes++;
    });
    ...
    return Object.values(agrupado).map(item => ({
        ...
        doador: 'Vários',
        categoria: item.categoria,
        item: `${item.doacoes} doações`,
        quantidade: item.quantidade,
        status: 'Agrupado',
        valorEstimado: item.valorTotal
    }));
}
...
function agruparPorDoador(dados) {
    ...
    dados.forEach(item => {
        if (!agrupado[item.doador]) {
            agrupado[item.doador] = {
                doador: item.doador,
                quantidade: 0,
                valorTotal: 0,
                doacoes: 0,
                ultimaDoacao: item.data
            };
        }
        
        agrupado[item.doador].quantidade += item.quantidade;
        agrupado[item.doador].valorTotal += (item.valorEstimado || 0);
        agrupado[item.doador].doacoes++;
        
        // Manter a data mais recente
        if (new Date(item.data) > new Date(agrupado[item.doador].ultimaDoacao)) {
            agrupado[item.doador].ultimaDoacao = item.data;
        }
    });
    ...
    return Object.values(agrupado)
        ...
        .map(item => ({
            data: item.ultimaDoacao,
            doador: item.doador,
            categoria: 'Várias',
            item: `${item.doacoes} doações`,
            quantidade: item.quantidade,
            status: 'Doador Frequente',
            valorEstimado: item.valorTotal
        }));
}

// Atualizar todas as visualizações - NOVA
...
// Função para exportar para PDF - ATUALIZADA
function exportarPDF() {
    ...
    setTimeout(() => {
        try {
            ...
            doc.text('Relatório de Doações - SOS Felino', margin, 20);
            ...
            doc.setFontSize(12);
            doc.setTextColor(44, 62, 80);
            doc.text('Resumo Estatístico', margin, 42);
            ...
            if (document.getElementById('valor-total')) {
                resumo.push(`Valor Total Estimado: ${document.getElementById('valor-total').textContent}`);
                resumo.push(`Categorias Diferentes: ${document.getElementById('categorias-unicas').textContent}`);
            }
            
            resumo.forEach((linha, index) => {
                doc.text(linha, margin, yPos + (index * 6));
            });
            
            yPos += (resumo.length * 6) + 15;
            
            // Preparar dados para a tabela
            const headers = [['Data', 'Doador', 'Categoria', 'Item', 'Quantidade', 'Valor (R$)', 'Status']];
            const tableData = filteredData.map(item => [
                ...
                (item.valorEstimado || 0).toLocaleString('pt-BR'),
                item.status
            ]);
            ...
            // Salvar o PDF
            doc.save(`relatorio_doacoes_${new Date().toISOString().split('T')[0]}.pdf`);
            
            mostrarNotificacao('PDF exportado com sucesso!', 'success');
        } catch (error) {
            console.error('Erro ao exportar PDF:', error);
            mostrarNotificacao('Erro ao exportar PDF. Verifique se todas as bibliotecas estão carregadas.', 'error');
        } finally {
            mostrarLoading(false);
        }
    }, 500);
...
// Função para exportar para Excel - ATUALIZADA
function exportarExcel() {
    ...
    setTimeout(() => {
        try {
            ...
            XLSX.writeFile(wb, `relatorio_doacoes_${new Date().toISOString().split('T')[0]}.xlsx`);
            
            mostrarNotificacao('Excel exportado com sucesso!', 'success');
        } catch (error) {
            console.error('Erro ao exportar Excel:', error);
            mostrarNotificacao('Erro ao exportar Excel. Verifique se todas as bibliotecas estão carregadas.', 'error');
        } finally {
            mostrarLoading(false);
        }
    }, 500);
...
function atualizarBreadcrumb() {
    ...
    if (breadcrumb && tipoRelatorio) {
        const textoRelatorio = tipoRelatorio.options[tipoRelatorio.selectedIndex].text;
        breadcrumb.textContent = `Relatórios - ${textoRelatorio}`;
    }
}

// Adicionar estilos CSS dinamicamente para novos elementos
